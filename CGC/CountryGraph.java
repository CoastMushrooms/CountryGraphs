...
public class CountryGraph {
    private static final String COUNTRY_BORDERS_FILE = "CountryBorders.CSV";
    private static final String SUBMERGED_BORDERS_FILE = "CountryBordersSubmerged.CSV";
    ...
    public List<Country> findPath(Country start, Country end, Set<Country> blockedCountries) {
        if (!countries.contains(start) || !countries.contains(end)) return new ArrayList<>();
        if (blockedCountries.contains(start) || blockedCountries.contains(end)) return new ArrayList<>();
    ...
    public void writeActiveBordersExcluding(Set<Country> submergedCountries) {
        Set<String> submergedNames = new HashSet<>();
        for (Country country : submergedCountries) {
            submergedNames.add(country.getName());
        }

        try (
                BufferedReader br = new BufferedReader(new FileReader(COUNTRY_BORDERS_FILE));
                PrintWriter writer = new PrintWriter(new FileWriter(SUBMERGED_BORDERS_FILE))
        ) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                String borderName = parts[3].trim();
                if (!submergedNames.contains(countryName) && !submergedNames.contains(borderName)) {
                    writer.println(line);
                }
            }
        } catch (IOException e) {
            System.err.println("Could not update " + SUBMERGED_BORDERS_FILE + ": " + e.getMessage());
        }
    }
